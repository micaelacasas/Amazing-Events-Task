...
const upcomingEvents = data.events.filter(events => events.date > data.currentDate)

function createTemplate (upcomingEvents, infoTemplate){
    infoTemplate.innerHTML = ""
    let template = ""
    upcomingEvents.forEach(evento => template += cardTemplate(evento)) 
    infoTemplate.innerHTML = template
}

console.log(upcomingEvents)
createTemplate(upcomingEvents, section)